...
void push (linkedlist* l,node* new_element); //stack
void pop(linkedlist*l,node*);       //stack
...
void main(void)
{
  int i ,x ,size ,Xpos=3 ,Ypos=1, tos=0;
  char key;
  node* ptr_node;
  linkedlist* l;
...
void push (linkedlist* l,node* new_element)
{
 add_node(l,new_element);
 /* if (l -> start == NULL)
  {
	l -> start = new_element;
	l -> end = new_element;
  ...
  else     //push always at the beginning
  {
...
void pop(linkedlist*l,node* ptr)
{
  node* temp;
  temp=ptr;
  temp = l -> start;
